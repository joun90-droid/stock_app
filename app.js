/* ==========================================================================
   전영재 전용 주식종목 찾기 - Core Application Script
   Target: 52개 대표 종목 데이터베이스 (코스피, 코스닥, 미국 대표 저평가주)
   ========================================================================== */
...
let appState = {
    ...
    savedIds: JSON.parse(localStorage.getItem('value_saved_stocks_yj') || '[]'),
    filters: {
        market: 'all',
        tag: 'all',
        searchQuery: '',
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-refresh real-time prices every 30 seconds
    setInterval(fetchStocksFromBackend, 30000);
});

function fetchStocksFromBackend() {
    fetch('/api/stocks')
        .then(res => res.json())
        .then(data => {
...
function initFilters() {
    const pbrRange = document.getElementById('maxPbrRange');
    const perRange = document.getElementById('maxPerRange');
    const roeRange = document.getElementById('minRoeRange');
    const divRange = document.getElementById('minDivRange');

    if (pbrRange) pbrRange.value = appState.filters.maxPbr;
    if (perRange) perRange.value = appState.filters.maxPer;
    if (roeRange) roeRange.value = appState.filters.minRoe;
    if (divRange) divRange.value = appState.filters.minDiv;

    updateRangeDisplays();
}

function updateRangeDisplays() {
    const pbrDisp = document.getElementById('maxPbrDisplay');
    const perDisp = document.getElementById('maxPerDisplay');
    const roeDisp = document.getElementById('minRoeDisplay');
    const divDisp = document.getElementById('minDivDisplay');

    if (pbrDisp) pbrDisp.textContent = `${parseFloat(appState.filters.maxPbr).toFixed(2)} 배 이하`;
    if (perDisp) perDisp.textContent = `${parseFloat(appState.filters.maxPer).toFixed(1)} 배 이하`;
    if (roeDisp) roeDisp.textContent = `${parseFloat(appState.filters.minRoe).toFixed(1)}% 이상`;
    if (divDisp) divDisp.textContent = `${parseFloat(appState.filters.minDiv).toFixed(1)}% 이상`;
}

function applyYeongjaePreset() {
    appState.filters.market = 'all';
    appState.filters.tag = 'all';
    appState.filters.searchQuery = '';
    appState.filters.maxPbr = 1.00;
    appState.filters.maxPer = 15.0;
    appState.filters.minRoe = 10.0;
    appState.filters.minDiv = 2.5;
    ...
    renderMainGrid();
    showToast('전영재 추천 픽 💖 (PBR ≤ 1.0배 · ROE ≥ 10% 필터 적용)');
}

function applyNasdaqPreset() {
    appState.filters.market = 'NASDAQ';
    appState.filters.tag = 'all';
    appState.filters.searchQuery = '';
    appState.filters.maxPbr = 30.00;
    appState.filters.maxPer = 70.0;
    appState.filters.minRoe = 0.0;
    appState.filters.minDiv = 0.0;
    appState.sortBy = 'score_desc';
    ...
    const marketPills = document.querySelectorAll('#marketPills .m-pill');
    marketPills.forEach(p => p.classList.toggle('active', p.dataset.market === 'NASDAQ'));

    const tagPills = document.querySelectorAll('#tagPills .t-pill');
    tagPills.forEach((p, idx) => p.classList.toggle('active', idx === 0));

    renderMainGrid();
    showToast('🇺🇸 전영재 추천 나스닥 저평가 50선이 로딩되었습니다! 🚀');
}

function initEventListeners() {
    const btnYeongjae = document.getElementById('btnPresetYeongjae');
    if (btnYeongjae) btnYeongjae.addEventListener('click', applyYeongjaePreset);

    const btnNasdaq = document.getElementById('btnPresetNasdaq');
    if (btnNasdaq) btnNasdaq.addEventListener('click', applyNasdaqPreset);
    ...
    marketPills.forEach(pill => {
        pill.addEventListener('click', () => {
            ...
            appState.filters.market = pill.dataset.market;
            renderMainGrid();
        });
    });
    ...
    tagPills.forEach(pill => {
        pill.addEventListener('click', () => {
            ...
            pill.classList.add('active');
            appState.filters.tag = pill.dataset.tag;
            renderMainGrid();
        });
    });
    ...
    if (pbrRange) {
        pbrRange.addEventListener('input', (e) => {
            appState.filters.maxPbr = parseFloat(e.target.value);
            updateRangeDisplays();
            renderMainGrid();
        });
    }
    ...
    if (perRange) {
        perRange.addEventListener('input', (e) => {
            appState.filters.maxPer = parseFloat(e.target.value);
            updateRangeDisplays();
            renderMainGrid();
        });
    }
    ...
    if (roeRange) {
        roeRange.addEventListener('input', (e) => {
            appState.filters.minRoe = parseFloat(e.target.value);
            updateRangeDisplays();
            renderMainGrid();
        });
    }
    ...
    if (divRange) {
        divRange.addEventListener('input', (e) => {
            appState.filters.minDiv = parseFloat(e.target.value);
            updateRangeDisplays();
            renderMainGrid();
        });
    }
    ...
    const btnCloseCalculator = document.getElementById('btnCloseCalculator');
    const btnRunCalc = document.getElementById('btnRunCalc');

    if (btnOpenCalculator) btnOpenCalculator.addEventListener('click', () => {
        document.getElementById('modalCalculator').classList.remove('hidden');
        runSRimCalculator();
    });
    if (btnCloseCalculator) btnCloseCalculator.addEventListener('click', () => {
        document.getElementById('modalCalculator').classList.add('hidden');
    });
    if (btnRunCalc) btnRunCalc.addEventListener('click', runSRimCalculator);

    const btnOpenScraper = document.getElementById('btnOpenScraper');
    const btnCloseScraper = document.getElementById('btnCloseScraper');
    const btnCloseScraperDone = document.getElementById('btnCloseScraperDone');
    const btnShareKakao = document.getElementById('btnShareKakao');

    if (btnOpenScraper) btnOpenScraper.addEventListener('click', runNaverScraperAnimation);
    if (btnShareKakao) btnShareKakao.addEventListener('click', shareToKakaoTalk);
    if (btnCloseScraper) btnCloseScraper.addEventListener('click', () => {
        document.getElementById('modalScraper').classList.add('hidden');
    });
    if (btnCloseScraperDone) btnCloseScraperDone.addEventListener('click', () => {
        document.getElementById('modalScraper').classList.add('hidden');
    });
    ...
    tabBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (activeTabElem) activeTabElem.classList.remove('hidden');
        });
    });
    ...
    if (btnDetailSave) {
        btnDetailSave.addEventListener('click', () => {
            if (appState.activeDetailId) toggleSaveStock(appState.activeDetailId);
        });
    }
    ...
    const btnExportSavedCsv = document.getElementById('btnExportSavedCsv');

    if (btnOpenSaved) btnOpenSaved.addEventListener('click', openSavedDrawer);
    if (btnCloseSaved) btnCloseSaved.addEventListener('click', closeSavedDrawer);
    if (btnClearSaved) btnClearSaved.addEventListener('click', clearAllSaved);
    if (btnExportSavedCsv) btnExportSavedCsv.addEventListener('click', exportSavedCsv);
...
function resetFilters() {
    ...
    document.querySelectorAll('#marketPills .m-pill').forEach((p, idx) => p.classList.toggle('active', idx === 0));
    document.querySelectorAll('#tagPills .t-pill').forEach((p, idx) => p.classList.toggle('active', idx === 0));
...
function getFilteredStocks() {
    return appState.stocks.filter(item => {
        if (appState.filters.market !== 'all') {
            if (appState.filters.market === 'NASDAQ') {
                if (item.marketName !== 'NASDAQ') return false;
            } else if (item.market !== appState.filters.market) {
                return false;
            }
        }
        if (appState.filters.tag !== 'all' && item.tag !== appState.filters.tag) return false;
        if (item.pbr > appState.filters.maxPbr) return false;
        if (item.per > appState.filters.maxPer) return false;
        if (item.roe < appState.filters.minRoe) return false;
        if (item.dividendYield < appState.filters.minDiv) return false;
        ...
        if (appState.filters.searchQuery) {
            ...
            if (!matchName && !matchCode && !matchEng && !matchDesc) return false;
        }

        return true;
    }).sort((a, b) => {
        if (appState.sortBy === 'score_desc') return b.valueScore - a.valueScore;
        if (appState.sortBy === 'upside_desc') return b.upsidePotential - a.upsidePotential;
        if (appState.sortBy === 'pbr_asc') return a.pbr - b.pbr;
        if (appState.sortBy === 'per_asc') return a.per - b.per;
        if (appState.sortBy === 'div_desc') return b.dividendYield - a.dividendYield;
        if (appState.sortBy === 'roe_desc') return b.roe - a.roe;
        return 0;
    });
}
...
function renderMainGrid() {
    ...
    if (visibleCountElem) visibleCountElem.textContent = filtered.length;
    if (activeFilterSummary) {
        let mLabel = '전체 시장';
        if (appState.filters.market === 'KR') mLabel = '🇰🇷 국내';
        else if (appState.filters.market === 'US') mLabel = '🇺🇸 미국 전체';
        else if (appState.filters.market === 'NASDAQ') mLabel = '🇺🇸 나스닥 50선';
        let summary = `${mLabel} · PBR ≤ ${appState.filters.maxPbr}배 · PER ≤ ${appState.filters.maxPer}배`;
        activeFilterSummary.textContent = summary;
    }
    ...
    grid.innerHTML = filtered.map(item => {
        const isSaved = appState.savedIds.includes(item.id);
        const isKr = item.market === 'KR';
        
        let priceFormatted = '';
        let fairPriceFormatted = '';
        const yahooUrl = isKr ? `https://finance.naver.com/item/main.naver?code=${item.code}` : `https://finance.yahoo.com/quote/${item.code}`;

        if (isKr) {
            priceFormatted = `${item.price.toLocaleString()} 원`;
            fairPriceFormatted = `${item.fairValue.toLocaleString()} 원`;
        } else {
            const krwPrice = Math.round(item.price * USD_KRW_EXCHANGE_RATE);
            const krwFair = Math.round(item.fairValue * USD_KRW_EXCHANGE_RATE);
            priceFormatted = `$${item.price.toFixed(2)} <span class="krw-sub">(약 ${krwPrice.toLocaleString()}원)</span>`;
            fairPriceFormatted = `$${item.fairValue.toFixed(2)} <span class="krw-sub">(약 ${krwFair.toLocaleString()}원)</span>`;
        }

        const changeClass = item.changeRate >= 0 ? 'up' : 'down';
        const changeSign = item.changeRate >= 0 ? '+' : '';

        return `
            <div class="stock-card" data-id="${item.id}" onclick="openDetailModal('${item.id}')">
                <div class="stock-card-header">
                    <div>
                        <span class="market-badge ${item.market}">${isKr ? '🇰🇷 ' : '🇺🇸 '}${item.marketName}</span>
                        <span class="sector-tag">${item.sector}</span>
                    </div>
                    <div class="score-badge">
                        <i class="fa-solid fa-gem"></i> 전영재 점수 ${item.valueScore}점
                    </div>
                </div>

                <div class="stock-title-box">
                    <h3>${item.name}</h3>
                    <p class="ticker-code">${item.code} · ${item.englishName}</p>
                </div>

                <div class="price-row">
                    <span class="price-val">${priceFormatted}</span>
                    <span class="change-badge ${changeClass}">${changeSign}${item.changeRate.toFixed(2)}%</span>
                </div>

                <div class="fair-value-box">
                    <div>
                        <span class="fair-label">S-RIM 적정주가</span>
                        <span class="fair-price">${fairPriceFormatted}</span>
                    </div>
                    <span class="upside-pill">+${item.upsidePotential.toFixed(1)}% 상승여력</span>
                </div>

                <div class="card-metrics-grid">
                    <div class="metric-item">
                        <span class="m-label">PBR</span>
                        <span class="m-val highlight">${item.pbr.toFixed(2)}배</span>
                    </div>
                    <div class="metric-item">
                        <span class="m-label">PER</span>
                        <span class="m-val">${item.per.toFixed(1)}배</span>
                    </div>
                    <div class="metric-item">
                        <span class="m-label">ROE</span>
                        <span class="m-val highlight">${item.roe.toFixed(1)}%</span>
                    </div>
                    <div class="metric-item">
                        <span class="m-label">배당률</span>
                        <span class="m-val">${item.dividendYield.toFixed(1)}%</span>
                    </div>
                </div>
                ...
                <div class="card-footer">
                    <a href="${item.naverUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-naver btn-sm" onclick="event.stopPropagation();" title="네이버 증권 시세">
                        <i class="fa-solid fa-arrow-up-right-from-square"></i> 네이버
                    </a>
                    ${!isKr ? `
                    <a href="${yahooUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-yahoo btn-sm" onclick="event.stopPropagation();" title="야후 파이낸스 실시간 시세">
                        <i class="fa-solid fa-globe"></i> 야후($)
                    </a>` : ''}
                    <button class="btn btn-calc btn-sm" onclick="event.stopPropagation(); openCalculatorForStock('${item.id}');" title="S-RIM 계산기">
                        <i class="fa-solid fa-calculator"></i> 계산기
                    </button>
                    <button class="btn btn-primary btn-sm" onclick="event.stopPropagation(); openDetailModal('${item.id}')">
                        <i class="fa-solid fa-chart-line"></i> 상세
                    </button>
                    <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation(); toggleSaveStock('${item.id}')" title="관심종목 저장">
                        <i class="fa-${isSaved ? 'solid' : 'regular'} fa-star" style="color: ${isSaved ? '#fbbf24' : 'inherit'}"></i>
                    </button>
                </div>
            </div>
        `;
...
function updateHeaderStats() {
    const countElem = document.getElementById('totalStocksCount');
    const upsideElem = document.getElementById('avgUpsideDisplay');
    const divElem = document.getElementById('avgDivDisplay');

    if (countElem) countElem.textContent = appState.stocks.length;

    if (appState.stocks.length > 0) {
        const avgUpside = appState.stocks.reduce((acc, cur) => acc + cur.upsidePotential, 0) / appState.stocks.length;
        const avgDiv = appState.stocks.reduce((acc, cur) => acc + cur.dividendYield, 0) / appState.stocks.length;
        if (upsideElem) upsideElem.textContent = `+${avgUpside.toFixed(1)}%`;
        if (divElem) divElem.textContent = `${avgDiv.toFixed(1)}%`;
    }
}
...
function toggleSaveStock(id) {
    const idx = appState.savedIds.indexOf(id);
    const item = appState.stocks.find(i => i.id === id);

    if (idx >= 0) {
        appState.savedIds.splice(idx, 1);
        showToast(`${item ? item.name : '종목'} 관심목록에서 삭제됨`);
    } else {
        appState.savedIds.push(id);
        showToast(`${item ? item.name : '종목'} 전영재 관심 포트폴리오에 저장됨 ⭐️`);
    }

    localStorage.setItem('value_saved_stocks_yj', JSON.stringify(appState.savedIds));
    updateSavedCount();
    renderMainGrid();
    if (document.getElementById('drawerSaved') && !document.getElementById('drawerSaved').classList.contains('hidden')) {
...
function openDetailModal(id) {
    ...
    appState.activeDetailId = id;

    const modal = document.getElementById('modalDetail');
    const header = document.getElementById('detailHeader');
    const kpis = document.getElementById('detailKpis');
    const financialsBox = document.getElementById('detailFinancialsBox');
    const rationaleGrid = document.getElementById('detailRationaleGrid');
    const btnSave = document.getElementById('btnDetailSave');
    const btnNaver = document.getElementById('btnDetailNaverUrl');
    const btnYahoo = document.getElementById('btnDetailYahooUrl');
    const btnCalc = document.getElementById('btnDetailCalc');
    ...
    if (isKr) {
        ...
        const krwPrice = Math.round(item.price * USD_KRW_EXCHANGE_RATE);
        const krwFair = Math.round(item.fairValue * USD_KRW_EXCHANGE_RATE);
        priceFormatted = `$${item.price.toFixed(2)} (약 ${krwPrice.toLocaleString()}원)`;
        fairPriceFormatted = `$${item.fairValue.toFixed(2)} (약 ${krwFair.toLocaleString()}원)`;
    }

    if (btnNaver) btnNaver.href = item.naverUrl;
    if (btnYahoo) {
        if (!isKr) {
            btnYahoo.href = `https://finance.yahoo.com/quote/${item.code}`;
            btnYahoo.style.display = 'inline-flex';
        } else {
            btnYahoo.style.display = 'none';
        }
    }
    if (btnCalc) {
        btnCalc.onclick = () => {
            closeDetailModal();
            openCalculatorForStock(item.id);
        };
    }
    ...
    if (header) {
        header.innerHTML = `
            <div class="detail-title-line">
                ...
                <span class="score-badge"><i class="fa-solid fa-gem"></i> 전영재 가치점수 ${item.valueScore}점</span>
            </div>
            <p class="sub">${item.englishName} · ${item.sector} · 시가총액 ${item.marketCap}</p>
        `;
    }
    ...
    if (kpis) {
        kpis.innerHTML = `
            <div class="kpi-card">
                ...
                <span class="val highlight">${priceFormatted}</span>
            </div>
            <div class="kpi-card">
                <span class="label">적정주가 (S-RIM)</span>
                <span class="val highlight-gold">${fairPriceFormatted}</span>
            </div>
            <div class="kpi-card">
                <span class="label">상승 여력</span>
                <span class="val highlight">+${item.upsidePotential.toFixed(1)}%</span>
            </div>
            <div class="kpi-card">
                <span class="label">PBR / PER</span>
                <span class="val">${item.pbr.toFixed(2)}배 / ${item.per.toFixed(1)}배</span>
            </div>
        `;
    }
        ...
        financialsBox.innerHTML = `
            <table class="financial-table">
                ...
                <tbody>
                    <tr>
                        <td>PBR (주가자산비율)</td>
                        <td>${(item.pbr * 1.15).toFixed(2)}배</td>
                        <td>${(item.pbr * 1.05).toFixed(2)}배</td>
                        <td>${item.pbr.toFixed(2)}배</td>
                        <td><strong style="color: var(--accent-green)">${item.pbr.toFixed(2)}배 (역사적 저평가)</strong></td>
                    </tr>
                    <tr>
                        <td>PER (주가수익비율)</td>
                        <td>${(item.per * 1.2).toFixed(1)}배</td>
                        <td>${(item.per * 1.1).toFixed(1)}배</td>
                        <td>${item.per.toFixed(1)}배</td>
                        <td><strong>${item.per.toFixed(1)}배</strong></td>
                    </tr>
                    <tr>
                        <td>ROE (자기자본이익률)</td>
                        <td>${(item.roe * 0.9).toFixed(1)}%</td>
                        <td>${(item.roe * 0.95).toFixed(1)}%</td>
                        <td>${item.roe.toFixed(1)}%</td>
                        <td><strong style="color: var(--accent-green)">${item.roe.toFixed(1)}% (고수익성)</strong></td>
                    </tr>
                    <tr>
                        <td>배당수익률</td>
                        <td>${(item.dividendYield * 0.85).toFixed(1)}%</td>
                        <td>${(item.dividendYield * 0.95).toFixed(1)}%</td>
                        <td>${item.dividendYield.toFixed(1)}%</td>
                        <td><strong>${item.dividendYield.toFixed(1)}%</strong></td>
                    </tr>
                </tbody>
            </table>
        `;
    ...
    if (rationaleGrid) {
        rationaleGrid.innerHTML = `
            <div class="rationale-box pros">
                <h4><i class="fa-solid fa-circle-check"></i> 전영재 투자 매력 (Pros)</h4>
                <ul>
                    ${item.pros.map(p => `<li>${p}</li>`).join('')}
                </ul>
            </div>
            <div class="rationale-box risks">
                <h4><i class="fa-solid fa-triangle-exclamation"></i> 주의 리스크 요인 (Risks)</h4>
                <ul>
    ...
    if (btnSave) {
        const isSaved = appState.savedIds.includes(item.id);
        btnSave.innerHTML = `<i class="fa-${isSaved ? 'solid' : 'regular'} fa-star"></i> ${isSaved ? '관심목록에서 삭제' : '관심종목 리스트에 저장'}`;
    }
...
function openCalculatorForStock(id) {
    const item = appState.stocks.find(s => s.id === id);
    if (!item) return;

    const modal = document.getElementById('modalCalculator');
    if (!modal) return;

    const bpsInput = document.getElementById('calcBps');
    const roeInput = document.getElementById('calcRoe');
    const priceInput = document.getElementById('calcCurrentPrice');
    const bpsLabel = document.getElementById('calcBpsLabel');
    const priceLabel = document.getElementById('calcPriceLabel');

    const isKr = item.market === 'KR';

    if (bpsInput) bpsInput.value = item.bps || 0;
    if (roeInput) roeInput.value = item.roe || 0;
    if (priceInput) priceInput.value = item.price || 0;

    if (bpsLabel) bpsLabel.textContent = isKr ? '현재 주당순자산 (BPS, 원)' : '현재 주당순자산 (BPS, $)';
    if (priceLabel) priceLabel.textContent = isKr ? '현재 주가 (원)' : '현재 주가 ($ USD)';

    modal.classList.remove('hidden');
    runSRimCalculator(item.market);
}
...
function renderDetailCharts(item) {
    const ctxVal = document.getElementById('chartValuationBand');
    const ctxRet = document.getElementById('chartShareholderReturn');

    if (!ctxVal || !ctxRet) return;

    if (appState.charts.valuationBand) appState.charts.valuationBand.destroy();
    if (appState.charts.shareholderReturn) appState.charts.shareholderReturn.destroy();

    appState.charts.valuationBand = new Chart(ctxVal, {
        type: 'line',
        data: {
            labels: ['2021년', '2022년', '2023년', '2024년', '현재'],
            datasets: [
                {
                    label: 'PBR (배)',
                    data: [item.pbr * 1.4, item.pbr * 1.25, item.pbr * 1.1, item.pbr * 1.05, item.pbr],
                    borderColor: '#10b981',
                    tension: 0.3,
                    fill: false
                },
                {
                    label: 'ROE (%)',
                    data: [item.roe * 0.85, item.roe * 0.9, item.roe * 0.95, item.roe * 0.98, item.roe],
                    borderColor: '#fbbf24',
                    borderDash: [5, 5],
                    tension: 0.3,
                    fill: false
    ...
    appState.charts.shareholderReturn = new Chart(ctxRet, {
        type: 'bar',
        data: {
            ...
            datasets: [
                {
                    label: '현금 배당금',
                    data: [item.dividendYield * 0.7, item.dividendYield * 0.85, item.dividendYield],
                    backgroundColor: 'rgba(16, 185, 129, 0.7)'
                },
                {
                    label: '자사주 소각 비율',
                    data: [item.dividendYield * 0.3, item.dividendYield * 0.4, item.dividendYield * 0.5],
...
function runSRimCalculator(overrideMarket) {
    const bps = parseFloat(document.getElementById('calcBps').value) || 0;
    const roe = parseFloat(document.getElementById('calcRoe').value) || 0;
    const discount = parseFloat(document.getElementById('calcDiscount').value) || 8.0;
    const currentPrice = parseFloat(document.getElementById('calcCurrentPrice').value) || 1;

    const excessReturn = roe - discount;
    const fairValue = bps + (bps * (excessReturn / discount));
    const upside = ((fairValue - currentPrice) / currentPrice) * 100;

    const resultElem = document.getElementById('calcFairResult');
    const upsideElem = document.getElementById('calcUpsideResult');

    const isKr = overrideMarket === 'KR' || (!overrideMarket && currentPrice > 1000);

    if (resultElem) {
        if (isKr) {
            resultElem.textContent = `${Math.round(fairValue).toLocaleString()} 원`;
        } else {
            const krwEquiv = Math.round(fairValue * USD_KRW_EXCHANGE_RATE);
            resultElem.textContent = `$${fairValue.toFixed(2)} (약 ${krwEquiv.toLocaleString()}원)`;
        }
    }
    if (upsideElem) {
        upsideElem.textContent = `${upside >= 0 ? '+' : ''}${upside.toFixed(1)}%`;
        upsideElem.style.color = upside >= 0 ? 'var(--accent-green)' : 'var(--accent-red)';
    }
}

function runNaverScraperAnimation() {
    const modal = document.getElementById('modalScraper');
    modal.classList.remove('hidden');

    const fill = document.getElementById('scanProgressFill');
    const status = document.getElementById('scanStatusText');
    const preview = document.getElementById('scannedResultsPreview');
    const btnDone = document.getElementById('btnCloseScraperDone');

    fill.style.width = '0%';
    status.textContent = '야후 파이낸스 & 네이버 증권 API 실시간 연결 중...';
    preview.classList.add('hidden');
    btnDone.classList.add('hidden');

    fetch('/api/refresh-live')
        .then(res => res.json())
        .then(res => {
    ...
    setTimeout(() => { fill.style.width = '30%'; status.textContent = 'KOSPI / KOSDAQ 밸류업 종목 실시간 체결가 파싱 중...'; }, 400);
    setTimeout(() => { fill.style.width = '70%'; status.textContent = '미국 NASDAQ / NYSE 50개 대표 종목 실시간 호가 파싱 중...'; }, 1000);
    setTimeout(() => {
        fill.style.width = '100%';
        status.textContent = '전종목 실시간 시세 및 밸류에이션 동기화 완료!';
        preview.classList.remove('hidden');
        btnDone.classList.remove('hidden');
        renderMainGrid();
        updateHeaderStats();
        showToast('🔴 전 종목 100% 실시간 시세 동기화 완료!');
    }, 1800);
}

function openSavedDrawer() {
    const drawer = document.getElementById('drawerSaved');
    renderSavedDrawerContent();
...
function renderSavedDrawerContent() {
    ...
    const countElem = document.getElementById('savedDrawerCount');
    const avgUpsideElem = document.getElementById('savedDrawerAvgUpside');
    const avgDivElem = document.getElementById('savedDrawerAvgDiv');

    const savedStocks = appState.stocks.filter(s => appState.savedIds.includes(s.id));

    if (countElem) countElem.textContent = `${savedStocks.length} 개`;

    if (savedStocks.length > 0) {
        const avgUpside = savedStocks.reduce((a, b) => a + b.upsidePotential, 0) / savedStocks.length;
        const avgDiv = savedStocks.reduce((a, b) => a + b.dividendYield, 0) / savedStocks.length;

        if (avgUpsideElem) avgUpsideElem.textContent = `+${avgUpside.toFixed(1)}%`;
        if (avgDivElem) avgDivElem.textContent = `${avgDiv.toFixed(1)}%`;
    } else {
        if (avgUpsideElem) avgUpsideElem.textContent = '+0.0%';
        if (avgDivElem) avgDivElem.textContent = '0.0%';
    }

    if (!container) return;

    if (savedStocks.length === 0) {
        container.innerHTML = `<p class="empty-drawer-text" style="color: var(--text-dim); text-align: center; padding: 40px 0;">저장된 관심 종목이 없습니다.</p>`;
        return;
    }

    container.innerHTML = savedStocks.map(item => `
        <div class="saved-item-row" style="display: flex; align-items: center; justify-content: space-between; background: rgba(255,255,255,0.03); padding: 12px; border-radius: 12px; border: 1px solid var(--border-color); margin-bottom: 8px;">
            <div>
                <strong style="color: white; font-size: 14px;">${item.name} (${item.code})</strong>
                <span style="display: block; font-size: 11px; color: var(--text-muted);">PBR ${item.pbr.toFixed(2)}배 · ROE ${item.roe}% · 상승여력 +${item.upsidePotential.toFixed(1)}%</span>
            </div>
            <button onclick="toggleSaveStock('${item.id}')" style="background: none; border: none; color: var(--accent-red); cursor: pointer; font-size: 16px;">
...
function clearAllSaved() {
    if (appState.savedIds.length === 0) return;
    appState.savedIds = [];
    localStorage.setItem('value_saved_stocks_yj', '[]');
    updateSavedCount();
    renderSavedDrawerContent();
    renderMainGrid();
    showToast('관심목록이 비워졌습니다.');
}

function exportSavedCsv() {
    const savedStocks = appState.stocks.filter(s => appState.savedIds.includes(s.id));
    if (savedStocks.length === 0) {
        showToast('저장된 종목이 없습니다.');
        return;
    }
    downloadCsv(savedStocks, '전영재_관심_저평가_포트폴리오.csv');
}
...
function exportFilteredCsv() {
    ...
    downloadCsv(filtered, '전영재_전용_저평가_주식종목_분석.csv');
}

function downloadCsv(list, filename) {
    const headers = ['시장', '종목명', '티커코드', '현재가', 'S-RIM적정주가', '상승여력(%)', 'PBR', 'PER', 'ROE(%)', '배당수익률(%)', '전영재가치점수', '네이버URL'];
    const rows = list.map(item => [
        ...
        item.price,
        item.fairValue,
        `${item.upsidePotential.toFixed(1)}%`,
        item.pbr,
        item.per,
        `${item.roe}%`,
        `${item.dividendYield}%`,
...
function showToast(msg) {
    ...
    toast.style.cssText = 'background: #1e293b; color: white; border: 1px solid var(--border-color-light); border-left: 4px solid var(--accent-green); padding: 12px 20px; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.5); font-size: 13px; display: flex; align-items: center; gap: 10px; margin-bottom: 8px;';
    toast.innerHTML = `<i class="fa-solid fa-sparkles" style="color: var(--accent-green)"></i> <span>${msg}</span>`;
...
function shareToKakaoTalk() {
    ...
    const shareData = {
        title: '📈 전영재 전용 주식종목 찾기 (실시간 시세 연동)',
        text: '🇰🇷국내 & 🇺🇸미국 저평가 93개 대표 주식 큐레이션 · S-RIM 적정주가 실시간 분석',
        url: currentUrl
    };

    if (navigator.share) {
        navigator.share(shareData)
            .then(() => showToast('카카오톡 / 모바일 공유창이 실행되었습니다! 💬'))
            .catch(() => copyShareUrlToClipboard());
    } else {
...
function copyShareUrlToClipboard() {
    ...
    if (navigator.clipboard) {
        navigator.clipboard.writeText(url)
            .then(() => showToast('카카오톡 공유 링크가 복사되었습니다! 📋 (카톡 대화창에 붙여넣어 보내세요)'))
            .catch(() => showToast('공유 링크: ' + url));
    } else {
        showToast('공유 링크: ' + url);
    }
}
